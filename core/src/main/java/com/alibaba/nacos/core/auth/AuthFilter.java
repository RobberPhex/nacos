...
public class AuthFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        } else if (StringUtils.isNotBlank(authConfigs.getServerIdentityValue())) {
            String serverIdentity = req.getHeader(authConfigs.getServerIdentityKey());
            if (StringUtils.equals(authConfigs.getServerIdentityValue(), serverIdentity)) {
                chain.doFilter(request, response);
                return;
            }
            if (StringUtils.isNotBlank(authConfigs.getServerIdentityValue2())
                    && StringUtils.equals(authConfigs.getServerIdentityValue2(), serverIdentity)) {
                chain.doFilter(request, response);
                return;
            }
